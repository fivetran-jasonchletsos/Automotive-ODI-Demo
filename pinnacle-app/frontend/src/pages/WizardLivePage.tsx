/*
 * WizardLivePage — dbt-wizard live-build playback for Pinnacle Motors.
 *
 ...
 *
 * Aesthetic: dark terminal surface (carbon). Fits a 1728x1117 laptop with no page scroll.
...
// Agent accent colors — Pinnacle Motors palette
...
export default function WizardLivePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(wizardDataUrl('wizard_agents.json')).then(r => {
        if (!r.ok) throw new Error(`Failed to fetch wizard_agents.json: ${r.status}`);
        return r.json();
      }),
      fetch(wizardDataUrl('wizard_scenario.json')).then(r => {
        if (!r.ok) throw new Error(`Failed to fetch wizard_scenario.json: ${r.status}`);
        return r.json();
      }),
      fetch(wizardDataUrl('wizard_build_script.json')).then(r => {
        if (!r.ok) throw new Error(`Failed to fetch wizard_build_script.json: ${r.status}`);
        return r.json();
      }),
    ]).then(([a, s, b]) => {
      setAgents(a.agents);
  ...
  if (!scenario || agents.length === 0 || events.length === 0) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-12">
        ...
        <p className="font-mono text-sm mt-2" style={{ color: 'var(--graphite-500)' }}>Loading build playback...</p>
      </div>
    );
  }
  ...
  return (
    <div className="wizard-terminal mx-auto max-w-[1640px] px-4 py-4 sm:px-6 lg:px-8">
      ...
      >
        <div className="flex items-center gap-3 flex-wrap">
          <span
            ...
            style={{
              ...
              background: 'rgba(220,38,38,0.12)', color: 'var(--t-accent)', border: '1px solid rgba(220,38,38,0.35)',
            }}
          >
            <span
              style={{
          ...
          <span className="font-mono" style={{ color: 'var(--t-text-dim)', fontSize: 13 }}>
            ...
            <span className="mx-2" style={{ color: 'var(--t-line)' }}>·</span>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            style={{ background: 'var(--t-elev)', borderColor: 'var(--t-line)', color: 'var(--t-text)', padding: '7px 14px', fontSize: 13, borderRadius: 0 }}
      ...
      <div
        className="mb-3 px-4 py-2.5 wiz-card border-l-4 flex items-center gap-5 flex-wrap"
        style={{ borderLeftColor: 'var(--t-accent)' }}
      >
        <div className="min-w-0 flex-shrink" style={{ flex: '1 1 460px' }}>
          <div className="font-condensed" style={{ fontSize: 10, marginBottom: 2, color: 'var(--t-accent)', letterSpacing: '0.16em' }}>
            Connected Vehicle Reliability · {scenario.timezone_label} · {scenario.requested_by}
          </div>
          <p
            className="font-display font-medium leading-snug truncate"
        ...
        <div className="font-mono shrink-0" style={{ color: 'var(--t-text-dim)', fontSize: 11 }}>
      ...
      <div className="mb-3 grid gap-1.5" style={{ gridTemplateColumns: 'repeat(6, minmax(0, 1fr))' }}>
        {STEP_DEFS.map((s, idx) => {
          ...
          const accentColor = active ? 'var(--t-accent)' : done ? '#4ade80' : 'var(--t-line)';
          return (
            <div
              key={s.label}
              className="wiz-card px-2.5 py-2 flex flex-col gap-0.5"
              style={{
                borderLeft: `4px solid ${accentColor}`,
                background: active ? 'rgba(220,38,38,0.08)' : done ? 'rgba(74,222,128,0.08)' : 'var(--t-surface)',
              }}
              title={`${s.who} · ${s.tools}`}
            >
              <div
                className="font-mono font-bold flex items-center gap-1.5"
                style={{ fontSize: 10, letterSpacing: '0.04em', color: active ? 'var(--t-accent)' : done ? '#4ade80' : 'var(--t-text-soft)' }}
              >
                <span>STEP {String(num).padStart(2, '0')}</span>
                <span style={{ opacity: 0.6 }}>·</span>
                <span>{done ? 'DONE' : active ? 'NOW' : 'WAIT'}</span>
              </div>
              <div className="font-display truncate" style={{ fontSize: 13, lineHeight: 1.15, color: 'var(--t-text)' }}>
                {s.label}
              </div>
              <div
      ...
      >
        ...
        <section
          className="wiz-card flex flex-col lg:!h-[calc(100dvh-440px)]"
        ...
        >
          ...
          >
            <div>
              <div className="font-condensed" style={{ fontSize: 11, color: 'var(--t-text-dim)', letterSpacing: '0.16em' }}>Sub-agent narration</div>
              <div className="font-mono mt-0.5" style={{ color: 'var(--t-text-dim)', fontSize: 12 }}>
          ...
          >
            {visibleNarr.map((m, idx) => {
              ...
              const color = a?.color ?? AGENT_STEP_COLOR[m.e.from] ?? '#dc2626';
                      ...
                      <div className="flex items-center gap-2 flex-wrap mb-1.5">
                        ...
                        <span
                          ...
                          style={{
                            ...
                            background: 'rgba(220,38,38,0.10)', color: 'var(--t-accent)',
                            border: '1px solid rgba(220,38,38,0.35)',
                          }}
                        >
                          STEP {m.e.step}
                        </span>
                        <span className="font-mono" style={{ color: 'var(--t-text-soft)', fontSize: 11 }}>
                          {m.e.step_label}
                        </span>
                      </div>
                      <div
        ...
        <section className="flex flex-col gap-3 lg:!h-[calc(100dvh-440px)]" style={{ minHeight: 'max(60vh, 300px)' }}>
          ...
          {/* SQL panel */}
          <div className="wiz-card flex flex-col" style={{ flex: '1.7 1 0' }}>
            <header
              className="px-5 py-3 border-b flex items-center justify-between"
              style={{ borderColor: 'var(--t-line)', background: 'var(--t-elev)' }}
            >
              <div className="flex items-center gap-3 flex-wrap min-w-0">
                <div className="font-mono" style={{ fontSize: 11, letterSpacing: '0.02em', color: 'var(--t-text-dim)' }}>
                  models/gold/fct_thermal_flags_by_powertrain_climate_week.sql
                </div>
                <span
                  className="layer-chip"
                  style={{
                    color: '#d97706', background: 'rgba(217,119,6,0.07)',
              ...
              <span className="font-mono" style={{ color: 'var(--t-text-soft)', fontSize: 12, whiteSpace: 'nowrap' }}>
            ...
            <pre
              ...
              style={{
                ...
                background: '#080c14', color: '#e8edf8',
            ...
            >
              {state.sqlSoFar.length === 0 ? (
                <span style={{ color: '#4a5568' }}>{'-- waiting for Worker to begin authoring...'}</span>
              ) : (
                <SyntaxSql
                  text={state.sqlSoFar}
                  cursor={
          ...
          {/* YAML panel */}
          <div className="wiz-card flex flex-col" style={{ flex: '1 1 0' }}>
            <header
              className="px-5 py-3 border-b flex items-center justify-between"
              style={{ borderColor: 'var(--t-line)', background: 'var(--t-elev)' }}
            >
              <div className="flex items-center gap-3 flex-wrap min-w-0">
                <div className="font-mono" style={{ fontSize: 11, letterSpacing: '0.02em', color: 'var(--t-text-dim)' }}>
                  models/gold/fct_thermal_flags_by_powertrain_climate_week.yml
                </div>
                <span
                  className="layer-chip"
                  style={{
            ...
            >
              {state.yamlSoFar.length === 0 ? (
                <span style={{ color: '#4a5568' }}>{'# waiting for Verification (step 5)...'}</span>
              ) : (
                <SyntaxYaml
      ...
      <div className="wiz-card mt-2 px-3 py-2 flex items-center gap-3">
        <div className="font-condensed shrink-0" style={{ fontSize: 10, color: 'var(--t-text-dim)', letterSpacing: '0.16em' }}>tool calls</div>
        {state.sideEffects.length === 0 ? (
          <div className="font-mono" style={{ color: 'var(--t-text-soft)', fontSize: 11.5 }}>Awaiting first tool call...</div>
        ) : (
      ...
      {complete && (
        ...
        >
          <div className="flex items-baseline justify-between flex-wrap gap-3 mb-1">
            ...
            <Link
              to="/wizard-outcome"
              className="inline-flex items-center gap-2 font-display tracking-wider transition-colors text-sm uppercase"
              style={{
                background: 'var(--t-accent)', color: '#fff',
                padding: '10px 18px', fontSize: 13, borderRadius: 0,
              }}
            >
              See the outcome
      ...
      <style>{`
        ...
        .wizard-terminal::after {
          content: 'pinnacle-motors/wizard-live · dbt-wizard';
          position: absolute;
          top: 0; left: 0; right: 0;
          height: 28px;
          display: flex;
        ...
        .wtok-com   { color: #5a7099; font-style: italic; }
        .wtok-num   { color: #d97706; }
        .wtok-jinja { color: #f59e0b; font-weight: 600; }

        /* Override eyebrow in dark context */
        .wizard-terminal .eyebrow {
          color: var(--t-accent) !important;
        }

        /* Status pills in dark context */
        ...
        .wizard-terminal a[class*="font-semibold"] {
          background: var(--t-elev) !important;
          color: var(--t-text) !important;
          border-color: var(--t-line) !important;
        }
        .wizard-terminal button:hover,
        .wizard-terminal a[class*="font-semibold"]:hover {
          border-color: var(--t-accent) !important;
        }

        /* Agent avatar in dark context */
        .wizard-terminal .wizard-agent-avatar {
          background: rgba(8,12,20,0.7) !important;
          border-color: rgba(100,130,170,0.35) !important;
        }
...
const SQL_KEYWORDS = new Set([
  ...
  'sum', 'max', 'min', 'avg', 'dateadd', 'date_trunc', 'current_date', 'is', 'not',
]);
